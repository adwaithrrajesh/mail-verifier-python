...
class Scout:
    def __init__(
        ...
        smtp_timeout: int = 2,  # Increased timeout for better success rate
    ...
    def get_optimal_smtp_port(self, domain: str) -> list:
        """Get optimal SMTP port order based on domain for faster verification"""
        # Gmail and Google Workspace work best with port 25
        if domain.endswith(('gmail.com', 'googlemail.com', 'google.com')):
            return [25]  # Only try 25 for Gmail
        # Outlook/Hotmail/Microsoft only work on port 25 for verification
        elif domain.endswith(('outlook.com', 'hotmail.com', 'live.com', 'msn.com', 'protection.outlook.com')):
            return [25]  # Only try 25 for Microsoft
        # Yahoo typically uses port 25
        elif domain.endswith(('yahoo.com', 'ymail.com', 'rocketmail.com')):
            return [25, 587]
        # Default: Try 587 first (most common for business), then 25
        return [587, 25]


    def get_optimal_port_for_mx(self, mx_host: str) -> list:
        """Get optimal SMTP port based on MX hostname"""
        mx_lower = mx_host.lower()
        # Google MX servers
        if 'google' in mx_lower or 'gmail' in mx_lower:
            return [25]
        # Microsoft MX servers
        elif 'outlook' in mx_lower or 'protection.outlook' in mx_lower or 'hotmail' in mx_lower:
            return [25]
        # Yahoo MX servers
        elif 'yahoo' in mx_lower or 'yahoodns' in mx_lower:
            return [25]
        # Default
        return [587, 25]
    ...
    def check_smtp(self, email: str, port: int = 25) -> Dict[str, Union[str, int, float, bool]]:
        ...
        try:
            ...
            # Port selection moved inside MX loop for better accuracy
            
            for mx in mx_hosts:
                server_key = f"{mx}"
                
                # Get optimal ports based on actual MX server
                smtp_ports = self.get_optimal_port_for_mx(mx)
                    ...
                    for smtp_port in smtp_ports:
                        ...
                        try:
                            ...
                            if smtp_port != 465 and server.has_extn('starttls'):
                                try:
                                    server.starttls()
                                    server.ehlo("blu-harvest.com")  # Re-EHLO after STARTTLS
                                except Exception:
